...
fn main() -> Status {
    ...
    loop {
        ...
        let pointer_handles = uefi::boot::find_handles::<uefi::proto::console::pointer::Pointer>().unwrap_or_default();
        let mut pointers = alloc::vec::Vec::new();
        for h in pointer_handles {
            if let Ok(p) = uefi::boot::open_protocol_exclusive::<uefi::proto::console::pointer::Pointer>(h) {
                pointers.push(p);
            }
        }
        ...
        if config.timeout == 0 && !config.entries.is_empty() {
            ...
            if use_gui {
                ...
                let mut gui = gui::GuiState {
                    ...
                    title: config.title.clone().map(alloc::string::String::from),
                    ..gui::GuiState::new()
                };
                let _ = gui.init();
        ...
        if let Some(entry) = selected_entry {
            if entry.kernel_path.is_none() {
                log::error!("Cannot boot '{}': no kernel specified.", entry.name);
                uefi::boot::stall(core::time::Duration::from_micros(3_000_000));
                continue;
            }
        ...
        // Reset timeout so it doesn't instantly select default on next loop iteration
        config.timeout = -1;
    }
}
